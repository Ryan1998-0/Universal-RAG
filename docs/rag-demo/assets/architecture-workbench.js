...
export const architectureViews = {
  ...
  ingestion: {
    ...
    width: 2100, height: 980,
    groups: [
      { label: "非同步文件處理主線", x: 35, y: 130, width: 2010, height: 380 },
      { label: "OCR 品質與受控學習閉環", x: 785, y: 560, width: 1260, height: 330 },
    ],
    nodes: [
      ...
      node("ing-parse", "04", "解析／OCR／VLM", "抽取文字、表格及圖片資訊。", "trusted document", "page elements + OCR trace", 820, 250,
        ["文字層與 OCR 自動分流", "圖片描述保留頁碼與區域", "抽樣檢查多模態品質"]),
      node("ing-ocr-gate", "04A", "OCR 品質 Gate", "依信心、數字／單位一致性、語言與版面訊號，決定可索引、待複核或重跑。", "OCR trace + page elements", "quality decision + issue record", 820, 650,
        ["保留 OCR 引擎、模型、DPI、語言與頁面雜湊", "數字、日期、表格欄位做一致性檢查", "低信心、失敗與高風險頁進入複核佇列"], { module: "OCR quality policy" }),
      node("ing-normalize", "05", "內容正規化", "統一文字與文件元素結構。", "page elements", "normalized elements", 1070, 250,
        ["保留條文與表格語意", "保留標題層級與定位"]),
      node("ing-review", "04B", "主動抽樣與人工校正", "優先校正低信心、資料漂移、數字密集及高使用率頁面。", "quality issues + sampled pages", "verified annotations", 1070, 650,
        ["抽樣規則覆蓋低信心與高影響資料", "校正文字、數字、單位、表格與區域定位", "每筆校正保留人員、規範與審核時間"], { module: "human-in-the-loop" }),
      node("ing-chunk", "06", "父子分塊", "建立可召回小塊與可回答父層上下文。", "normalized elements", "parent／child chunks", 1320, 250,
        ["依文件結構切分", "避免答案與條件分離", "保存 parent 關聯"]),
      node("ing-gold", "04C", "版本化金標資料集", "將人工確認標註組成可追溯的 train／validation／holdout 集。", "verified annotations", "dataset version", 1320, 650,
        ["以文件家族切分資料，避免同版面洩漏", "保存原始頁面、區域、正規答案與標註規範", "偽標籤只能當候選，未審核不得進金標"], { module: "OCR dataset registry" }),
      node("ing-embed", "07", "Embedding", "將 chunks 轉成語意向量。", "child chunks", "vectors", 1570, 250,
        ["記錄模型版本", "驗證批次處理與重試"]),
      node("ing-eval", "04D", "候選 OCR 離線評測", "以凍結 holdout 比較 OCR 與版面模型，決定是否可升版。", "candidate model + dataset version", "promotion decision", 1570, 650,
        ["量測 CER、數字 exact match、表格 cell F1 與頁面召回", "所有文件類型不可出現未核准退步", "輸出可重現的 raw prediction、score 與失敗頁"], { module: "offline evaluation" }),
      node("ing-index", "08", "持久化索引", "保存 BM25、向量與 metadata 關聯。", "chunks、vectors、metadata", "searchable KB", 1820, 250,
        ["權限資訊寫入所有索引", "索引可由原始資料重建"]),
      node("ing-release", "04E", "Shadow／灰度發布", "先以影子流量與可回滾版本驗證候選 OCR，再逐步取代現行策略。", "approved candidate + production telemetry", "active OCR policy", 1820, 650,
        ["候選輸出不覆蓋現行索引與原始資料", "監控品質漂移、成本、延遲與 Evidence Gate 失敗率", "只有通過門檻才升版，否則自動回滾"], { module: "controlled release" }),
    ],
    edges: [
      ["ing-file", "ing-validate"], ["ing-validate", "ing-security"], ["ing-security", "ing-parse"],
      ["ing-parse", "ing-ocr-gate"], ["ing-ocr-gate", "ing-normalize"], ["ing-normalize", "ing-chunk"],
      ["ing-chunk", "ing-embed"], ["ing-embed", "ing-index"],
      ["ing-ocr-gate", "ing-review"], ["ing-review", "ing-gold"], ["ing-gold", "ing-eval"],
      ["ing-eval", "ing-release"], ["ing-release", "ing-parse"],
    ],
  },
  ...
  retrieval: {
    ...
    nodes: [
      ...
      node("ret-rrf", "FUSION", "加權 RRF 融合", "以 BM25 0.6 與 Dense 0.4 的 reciprocal rank fusion 合併兩個排序清單。", "branch ranks", "fused candidates", 960, 360,
        ["驗證多列表排序", "保留各分支診斷分數", "RRF k=60"]),
