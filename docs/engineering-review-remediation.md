...
| 8 | Demo duplicate route/retrieve and ask | Code changed; live browser check pending | Local Demo now sends one `/api/ask` request per answer and renders route, contexts, evidence evaluation, run ID and timings from that response. Document-selection reruns use the same path; changing language only rerenders saved evidence. JavaScript syntax and response-contract checks pass. A local browser run with Ollama is still needed. |
